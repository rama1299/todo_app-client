...
import { getListDetail } from "@/modules/fetchList";
import { updateTaskStatus, updateTaskPriority, deleteTask } from "@/modules/fetchTask";
...
function TaskTable({ id }) {
    ...
    <div className="sm:flex items-center justify-between">
      <div className="flex items-center">
        <div className="py-2 px-8 text-2xl">
          ...
          <p className=" font-medium text-2xl">{listName}</p>
          <svg onClick={()=>{router.push(`/app/lists/update/${listId}`)}} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className="w-4 h-4 ml-2 text-blue-400 cursor-pointer hover:text-blue-600">
  ...
  <path d="M3.5 5.75c0-.69.56-1.25 1.25-1.25H10A.75.75 0 0010 3H4.75A2.75 2.75 0 002 5.75v9.5A2.75 2.75 0 004.75 18h9.5A2.75 2.75 0 0017 15.25V10a.75.75 0 00-1.5 0v5.25c0 .69-.56 1.25-1.25 1.25h-9.5c-.69 0-1.25-.56-1.25-1.25v-9.5z" />
      ...
      <button
        onClick={() => {router.push(`/app/tasks/add/${listId}`)}}
            ...
            {tasks.map((task) => (
              <React.Fragment key={task.id}>
                ...
                >
                    ...
                    <div className="ml-5">
                      <div className="rounded-sm w-5 h-5 flex flex-shrink-0 justify-center items-center relative">
                        {task.status === 'complete' ? (
                          ...
                          <svg
                            className="h-8 w-8 text-gray-300 cursor-pointer hover:text-green-200"
                            onClick={() => handleCheckbox(task.id, task.status)}
                            width="24"
                            height="24"
                            viewBox="0 0 24 24"
                  ...
                  <td>
                  <svg onClick={() => handleDelete(task.id)} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className="hover:text-gray-600 cursor-pointer ml-2 w-5 h-5 text-gray-200">
